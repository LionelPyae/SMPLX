...
def main(model_folder,
         ...
         plot_joints=False,
         num_betas=200,
         sample_shape=False, # Set to False to load custom betas
         sample_expression=False, # Set to False to load custom expression
    ...
    image = cv2.imread(image_path)[:,:,::-1] # Convert from BGR to RGB
    image_height, image_width = image.shape[:2]
    image = resize(image, (224, 224), anti_aliasing=True) # Resize to 224x224
    image = image.astype(np.float32) / 255.0 # Normalize to [0, 1]
    image = torch.from_numpy(image.transpose((2, 0, 1))) # Convert to PyTorch tensor
    image = image.unsqueeze(0) # Add batch dimension
    print(image.shape)

    # Extract pose and shape parameters from input image
    with torch.no_grad():
        output = model(image)
        print('output', output)
        betas = output.betas.detach().cpu().numpy()
        expression = output.expression.detach().cpu().numpy()
    print('betas', betas)
